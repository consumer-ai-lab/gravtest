...
function AdminViewSubmission() {
    ...
    function StaticTable(props) {
        return <div className="container" style={{ width: '70%', margin: 'auto' }}>
           <p style={{textAlign:'center',fontWeight:'bolder'}}> Select the Prefered Slot and click the Fetch Batch Data Button</p>
        </div>
    }
                ...
                <tbody>
                    {
                        batchData.batchData.map((b) => (
                            <tr>
                                ...
                                <td>
                                {b[2] === "Present" ?
                                       <center><Button variant="contained" color="primary" endIcon={<DownloadIcon />} onClick={() => downloadSingleFile(b[0], b[1])}>Download</Button></center>
                                        :
    ...
    const logoutMethod = () => {
        const token = localStorage.getItem("token") || '';
        axios.post(BASE_URL + "auth/logoutAdmin", {
    ...
    const fetchBatchData = async () => {
        console.log(batch)
        await axios.get(BASE_URL + `userData/get_slotWise_list_frontend/?batch=${batch}`).
            then((response) => {
                setBatchData(response.data)
            })
        console.log(batchData)
    ...
    const downloadAllFiles = async () => {
        ...
        for (let i = 0; i < batchData.length; i++) {
            if (batchData[i][1] !== "" && batchData[i][1] !== null && typeof (batchData[i][1]) !== "undefined") {
                await axios.get(BASE_URL + `s3download/single_file/?username=${batchData[i][0]}`, {
                    responseType: 'blob'
                }).
                    then((response) => {
    ...
    const downloadSingleFile = async (username, fileId) => {
        ...
        if (fileId !== "" && fileId !== null && typeof (fileId) !== "undefined") {
            await axios.get(BASE_URL + `s3download/single_file/?username=${username}`, {
                responseType: 'blob'
            }).
                then((response) => {
                    const downloadUrl = window.URL.createObjectURL(new Blob([response.data]));
        ...
        <div>
            ...
            <div className="container">
                <Collapse in={openSuccessAlert}>
                    <Alert
                        style={{ position: 'absolute', zIndex: '1', marginLeft: '750px', marginTop: '10px' }}
                        severity="success"
                        variant="filled"
                        size="medium"
                        action={
                ...
                <div className="col-8  mt-5 mb-5 p-3 bg-light" style={{
                    margin: 'auto',
                }}>
                    <h4 style={{ textAlign: 'center', padding: '10px',color:'black' }}> Download All Answer Sheets</h4>
                    <div className="timeSlot mb-5" style={{ width: '60%', marginLeft: '20px' }}>
                        <h5 style={{ marginLeft: '10px' }}>STEP 1 : Please Select the Timeslot</h5>
                        <select style={{ width: '100%', border: 'none', padding: '10px', borderRadius: '20px' }} name="Select Time Slot" onChange={(e) => setBatch(e.target.value)}>
                    ...
                    <div className="timeSlot mb-5" style={{ width: '60%', marginLeft: '20px' }}>
                        <h5 style={{ marginLeft: '10px', paddingBottom: '10px' }}>STEP 2 : Fetch Corresponding Batch Details</h5>
                        <div className="col-md-6 col-12" style={{ marginLeft: '20px' }}>
                            <Button variant="contained" color="primary" onClick={fetchBatchData}>Fetch Batch Data</Button>
                        </div>
                    </div>
                    <div className="timeSlot mb-5 pd-3" style={{ width: '100%', marginLeft: '20px' }}>
                        <h5 style={{ marginLeft: '10px', paddingBottom: '10px' }}>STEP 3 : Click on the below button to download all submission</h5>
                        <div className="col-md-6 col-12">
                            <Button style={{ marginLeft: '20px' }} variant="contained" color="primary" endIcon={<DownloadIcon />} onClick={downloadAllFiles}>DownLoad All Submission</Button>
                        </div>
                    </div>

                </div>
                {batchData.length === 0 ? <StaticTable /> : <DynamicTable batchData={batchData} />}
            </div>
        </div>
